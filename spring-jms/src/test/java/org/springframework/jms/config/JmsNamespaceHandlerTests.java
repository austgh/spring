...
public class JmsNamespaceHandlerTests {
	...
	public void testJmsContainerFactoryConfiguration() {
		...
		assertTrue("Wrong pub/sub", container.isPubSubDomain());
		assertTrue("Wrong durable flag", container.isSubscriptionDurable());
	...
	public void testResponseDestination() {
		...
		DefaultMessageListenerContainer listener2 = this.context
				.getBean("listener2", DefaultMessageListenerContainer.class);
		assertTrue("Wrong destination type on listener1", listener1.isPubSubDomain());
		assertTrue("Wrong destination type on listener2", listener2.isPubSubDomain());
		assertFalse("Wrong response destination type on listener1", listener1.isReplyPubSubDomain());
		assertFalse("Wrong response destination type on listener2", listener2.isReplyPubSubDomain());
		...
		JmsMessageEndpointManager listener4 = this.context
				.getBean("listener4", JmsMessageEndpointManager.class);
		assertTrue("Wrong destination type on listener3", listener3.isPubSubDomain());
		assertTrue("Wrong destination type on listener4", listener4.isPubSubDomain());
		assertFalse("Wrong response destination type on listener3", listener3.isReplyPubSubDomain());
		assertFalse("Wrong response destination type on listener4", listener4.isReplyPubSubDomain());
	}
