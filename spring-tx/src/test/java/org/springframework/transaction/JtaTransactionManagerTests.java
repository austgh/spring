...
public class JtaTransactionManagerTests {
				...
				protected void doInTransactionWithoutResult(TransactionStatus status) {
					...
					TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronizationAdapter() {
						...
						public void afterCompletion(int status) {
                            assertEquals("Correct completion status", TransactionSynchronization.STATUS_ROLLED_BACK,
                                    status);
						}
					});
				}
	...
	public void jtaTransactionManagerWithHeuristicMixedExceptionOnCommit() throws Exception {
		...
		try {
			...
			tt.execute(new TransactionCallbackWithoutResult() {
				...
				protected void doInTransactionWithoutResult(TransactionStatus status) {
					...
					TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronizationAdapter() {
						@Override
						public void afterCompletion(int status) {
                            assertEquals("Correct completion status", TransactionSynchronization.STATUS_UNKNOWN, status);
						}
					});
				}
			});
			...
			// expected
            assertEquals(HeuristicCompletionException.STATE_MIXED, ex.getOutcomeState());
		}
	...
	public void jtaTransactionManagerWithHeuristicRollbackExceptionOnCommit() throws Exception {
		...
		try {
			...
			// expected
            assertEquals(HeuristicCompletionException.STATE_ROLLED_BACK, ex.getOutcomeState());
		}
