...
public class CastorUnmarshallerTests extends AbstractUnmarshallerTests<CastorMarshaller> {
	...
	public void unmarshalStreamSourceWithXmlOptions() throws Exception {
		...
		assertTrue(result.get().getFeature("http://apache.org/xml/features/disallow-doctype-decl"));
		assertFalse(result.get().getFeature("http://xml.org/sax/features/external-general-entities"));
		...
		marshaller.unmarshal(new StreamSource("1"));
		assertNotNull(result.get());
		assertFalse(result.get().getFeature("http://apache.org/xml/features/disallow-doctype-decl"));
		assertTrue(result.get().getFeature("http://xml.org/sax/features/external-general-entities"));
	}
