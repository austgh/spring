...
public class StringUtilsTests {
	...
	public void testHasTextBlank() {
		String blank = "          ";
        assertFalse(StringUtils.hasText(blank));
	}
	...
	public void testCountOccurrencesOf() {
        assertEquals("nullx2 = 0", 0, StringUtils.countOccurrencesOf(null, null));
        assertEquals("null string = 0", 0, StringUtils.countOccurrencesOf("s", null));
        assertEquals("null substring = 0", 0, StringUtils.countOccurrencesOf(null, "s"));
		String s = "erowoiueoiur";
        assertEquals("not found = 0", 0, StringUtils.countOccurrencesOf(s, "WERWER"));
        assertEquals("not found char = 0", 0, StringUtils.countOccurrencesOf(s, "x"));
        assertEquals("not found ws = 0", 0, StringUtils.countOccurrencesOf(s, " "));
        assertEquals("not found empty string = 0", 0, StringUtils.countOccurrencesOf(s, ""));
        assertEquals("found char=2", 2, StringUtils.countOccurrencesOf(s, "e"));
        assertEquals("found substring=2", 2, StringUtils.countOccurrencesOf(s, "oi"));
        assertEquals("found substring=2", 2, StringUtils.countOccurrencesOf(s, "oiu"));
        assertEquals("found substring=3", 1, StringUtils.countOccurrencesOf(s, "oiur"));
        assertEquals("test last", 2, StringUtils.countOccurrencesOf(s, "r"));
	}
	...
	public void testReplace() {
        ...
        assertEquals("Replace 1 worked", "a6AazAfoo77abfoo", s);
	...
	public void testDelete() {
        ...
        assertEquals("Result has no the [" + noThe + "]", "The quick brown fox jumped over  lazy dog", noThe);

		String nohe = StringUtils.delete(inString, "he");
        assertEquals("Result has no he [" + nohe + "]", "T quick brown fox jumped over t lazy dog", nohe);

		String nosp = StringUtils.delete(inString, " ");
        assertEquals("Result has no spaces", "Thequickbrownfoxjumpedoverthelazydog", nosp);

		String killEnd = StringUtils.delete(inString, "dog");
        assertEquals("Result has no dog", "The quick brown fox jumped over the lazy ", killEnd);

		String mismatch = StringUtils.delete(inString, "dxxcxcxog");
        assertEquals("Result is unchanged", mismatch, inString);

		String nochange = StringUtils.delete(inString, "");
        assertEquals("Result is unchanged", nochange, inString);
	}
	...
	public void testDeleteAny() {
        ...
        assertEquals("Result has no Is [" + res + "]", "Able was  ere  saw Elba", res);

		res = StringUtils.deleteAny(inString, "AeEba!");
        assertEquals("Result has no Is [" + res + "]", "l ws I r I sw l", res);
        ...
        assertFalse("Has no CR", cleaned.contains("\n"));
        assertFalse("Has no tab", cleaned.contains("\t"));
        assertFalse("Has no sp", cleaned.contains(" "));
	...
	public void testGetFilenameExtension() {
        ...
        assertNull(StringUtils.getFilenameExtension(""));
        assertNull(StringUtils.getFilenameExtension("myfile"));
        assertNull(StringUtils.getFilenameExtension("myPath/myfile"));
        assertNull(StringUtils.getFilenameExtension("/home/user/.m2/settings/myfile"));
	...
	public void testCommaDelimitedListToStringArrayWithNullProducesEmptyArray() {
        ...
        assertNotNull("String array isn't null with null input", sa);
        assertEquals("String array length == 0 with null input", 0, sa.length);
	}
	...
	public void testCommaDelimitedListToStringArraySingleString() {
        ...
        assertEquals("Found one String with no delimiters", 1, sa.length);
        assertEquals("Single array entry matches input String with no delimiters", sa[0], s);
	}
	...
	private void doTestCommaDelimitedListToStringArrayLegalMatch(String[] components) {
        ...
        assertNotNull("String array isn't null with legal match", sa);
		assertEquals("String array length is correct with legal match", components.length, sa.length);
        assertArrayEquals("Output equals input", sa, components);
	}
