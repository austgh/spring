...
public class JndiTemplateEditorTests {
	...
	public void testCustomEnvironment() {
		...
		JndiTemplate jt = (JndiTemplate) je.getValue();
		assertEquals(2, jt.getEnvironment().size());
		assertEquals("org.springframework.myjndi.CompleteRubbish", jt.getEnvironment().getProperty(
				"jndiInitialSomethingOrOther"));
		assertEquals("bar", jt.getEnvironment().getProperty("foo"));
	}
