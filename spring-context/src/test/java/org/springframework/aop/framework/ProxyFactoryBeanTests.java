...
public class ProxyFactoryBeanTests {
	...
	public void testPrototypeInstancesAreNotEqual() {
		...
		assertNotSame("Prototype instances !=", test2, test2_1);
		assertEquals("Prototype instances equal", test2, test2_1);
		assertTrue("Has correct object type", ITestBean.class.isAssignableFrom(factory.getType("prototype")));
	}
	...
	private Object testPrototypeInstancesAreIndependent(String beanName) {
		...
		assertNotSame("Prototypes are not ==", prototype2FirstInstance, prototype2SecondInstance);
	...
	public void testAutoInvoker() {
		...
		assertEquals(autoInvoker.getName(), name);
	}
	...
	public void testCanAddAndRemoveAspectInterfacesOnPrototype() {
		...
		assertEquals(config.getAdvisors().length, oldCount + 1);
		...
		assertEquals(config.getAdvisors().length, oldCount);

		// Check no change on existing object reference
		assertEquals(ts.getTimeStamp(), time);
		...
		assertEquals(0, debugInterceptor.getCount());
	...
	public void testMethodPointcuts() {
		...
		assertEquals("setAge", PointcutForVoid.methodNames.get(0));
		assertEquals("setName", PointcutForVoid.methodNames.get(1));
	}
	...
	*/
	...
	public void testGlobalsCanAddAspectInterfaces() {
		...
		assertEquals(-1, agi.globalsAdded());
	...
	public static class GlobalIntroductionAdvice implements IntroductionAdvisor {
		...
		private final IntroductionInterceptor gi = new GlobalAspectInterfaceInterceptor();
