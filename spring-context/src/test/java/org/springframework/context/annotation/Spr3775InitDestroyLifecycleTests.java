...
public class Spr3775InitDestroyLifecycleTests {
	...
	public void testInitDestroyMethods() {
		...
		assertMethodOrdering(beanClass, "init-methods", Collections.singletonList("afterPropertiesSet"), bean.initMethods);
		beanFactory.destroySingletons();
		assertMethodOrdering(beanClass, "destroy-methods", Collections.singletonList("destroy"), bean.destroyMethods);
	}
	...
	public void testInitializingDisposableInterfacesWithShadowedMethods() {
		...
		assertMethodOrdering(beanClass, "init-methods", Collections.singletonList("InitializingBean" +
						".afterPropertiesSet"),
				bean.initMethods);
		beanFactory.destroySingletons();
		assertMethodOrdering(beanClass, "destroy-methods", Collections.singletonList("DisposableBean.destroy"), bean.destroyMethods);
	}
