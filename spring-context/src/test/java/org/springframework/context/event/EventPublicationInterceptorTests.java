...
public class EventPublicationInterceptorTests {
	...
	public void testExpectedBehavior() throws Exception {
		...
		assertEquals("Interceptor must have published 2 events", 2, listener.getEventCount());
		TestListener otherListener = (TestListener) ctx.getBean("&otherListener");
		assertEquals("Interceptor must have published 2 events", 2, otherListener.getEventCount());
	}
