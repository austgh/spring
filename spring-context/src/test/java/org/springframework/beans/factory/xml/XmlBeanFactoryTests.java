...
public class XmlBeanFactoryTests {
	...
	public void testRefToSeparatePrototypeInstances() {
		...
		assertNotSame("Emma and georgia think they have a different boyfriend", emmasJenks, georgiasJenks);
		assertEquals("Emmas jenks has right name", "Andrew", emmasJenks.getName());
		assertNotSame("Emmas doesn't equal new ref", emmasJenks, xbf.getBean("jenks"));
		assertEquals("Georgias jenks has right name", "Andrew", emmasJenks.getName());
		assertEquals("They are object equal", emmasJenks, georgiasJenks);
		assertEquals("They object equal direct ref", emmasJenks, xbf.getBean("jenks"));
	}
	...
	public void testRefToSingleton() {
		...
		assertSame("1 jen instance", davesJen, jenksJen);
		assertSame("1 jen instance", davesJen, jen);
	}
	...
	public void testInnerBeans() throws IOException {
		...
		assertNull(innerFactory.getName());
	...
	public void testInheritanceFromParentFactoryPrototype() {
		...
		assertEquals("override", inherits.getName());
		// Age property is inherited from bean in parent factory
		assertEquals(1, inherits.getAge());
		TestBean inherits2 = (TestBean) child.getBean("inheritsFromParentFactory");
		assertNotSame(inherits2, inherits);
	}
	...
	public void testPrototypeInheritanceFromParentFactoryPrototype() {
		...
		// Name property value is overridden
		assertEquals("prototype-override", inherits.getName());
		// Age property is inherited from bean in parent factory
		assertEquals(2, inherits.getAge());
		TestBean inherits2 = (TestBean) child.getBean("prototypeInheritsFromParentFactoryPrototype");
		assertNotSame(inherits2, inherits);
		inherits2.setAge(13);
		assertEquals(13, inherits2.getAge());
	...
	public void testPrototypeInheritanceFromParentFactorySingleton() {
		...
		// Name property value is overridden
		assertEquals("prototypeOverridesInheritedSingleton", inherits.getName());
		// Age property is inherited from bean in parent factory
		assertEquals(1, inherits.getAge());
	...
	public void testChildOverridesParentBean() {
		...
		// Name property value is overridden
		assertEquals("overrideParentBean", inherits.getName());
		// Age property is inherited from bean in parent factory
		assertEquals(1, inherits.getAge());
	...
	public void testSingletonFromParent() {
		...
		assertSame("singleton from parent and child is the same", beanFromParent, beanFromChild);
	}
	...
	public void testCircularReferences() {
		...
		assertSame("Correct circular reference", jenny.getSpouse(), david);
		assertSame("Correct circular reference", david.getSpouse(), jenny);
		assertSame("Correct circular reference", ego.getSpouse(), ego);
		assertSame("Correct circular reference", complexInnerEgo.getSpouse().getSpouse(), complexInnerEgo);
		assertSame("Correct circular reference", complexEgo.getSpouse().getSpouse(), complexEgo);
	}
	...
	public void testCircularReferenceWithTwoFactoryBeans() {
		...
		assertSame("Correct circular reference", ego1.getSpouse().getSpouse(), ego1);
		TestBean ego3 = (TestBean) xbf.getBean("ego3");
		assertSame("Correct circular reference", ego3.getSpouse().getSpouse(), ego3);
	}
	...
	public void testCircularReferencesWithWrappingAndRawInjectionAllowed() {
		...
		assertFalse(AopUtils.isAopProxy(david.getSpouse()));
	}
	...
	public void testFactoryReferenceCircle() {
		...
		assertSame(tb, db.getOtherTestBean());
	}
	...
	public void testAutowireWithDefault() {
		...
		assertEquals("Kerry", rod1.getSpouse().getName());

		DependenciesBean rod2 = (DependenciesBean) xbf.getBean("rod2");
		// should have been autowired
		assertNotNull(rod2.getSpouse());
		assertEquals("Kerry", rod2.getSpouse().getName());
	}
	...
	public void testReplaceMethodOverrideWithSetterInjection() {
		...
		assertFalse("Jenny".equals(notJenny.getName()));
