...
public class WebRequestDataBinderTests {
	...
	public void testPrefix() throws Exception {
		...
		ServletRequestParameterPropertyValues pvs = new ServletRequestParameterPropertyValues(request);
		assertFalse("Didn't find normal when given prefix", pvs.contains("forname"));
	...
	protected void doTestTony(PropertyValues pvs) throws Exception {
		...
		for (PropertyValue pv : pvArray) {
			...
			assertEquals("val matches expected", val, pv.getValue());
			m.remove(pv.getName());
		}
	...
	public void testNoParameters() throws Exception {
		MockHttpServletRequest request = new MockHttpServletRequest();
		ServletRequestParameterPropertyValues pvs = new ServletRequestParameterPropertyValues(request);
		assertEquals("Found no parameters", 0, pvs.getPropertyValues().length);
	}
	...
	public void testMultipleValuesForParameter() throws Exception {
		...
		ServletRequestParameterPropertyValues pvs = new ServletRequestParameterPropertyValues(request);
		assertEquals("Found 1 parameter", 1, pvs.getPropertyValues().length);
