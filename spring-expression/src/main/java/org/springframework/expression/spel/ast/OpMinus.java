		...
		if (this.children.length < 2) {  // if only one operand, then this is unary minus
			...
			if (operand instanceof Number) {
				if (operand instanceof BigDecimal) {
					...
					return new TypedValue(-((Number) operand).longValue());
				}
				else if (operand instanceof Integer) {
					this.exitTypeDescriptor = "I";
					return new TypedValue(-((Number) operand).intValue());
				}
				else if (operand instanceof Short) {
					return new TypedValue(-((Number) operand).shortValue());
				}
				else if (operand instanceof Byte) {
					return new TypedValue(-((Number) operand).byteValue());
				}
				else {
