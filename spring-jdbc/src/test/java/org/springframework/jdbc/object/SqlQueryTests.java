...
public class SqlQueryTests  {
	...
	public void testQueryWithoutParams() throws SQLException {
		...
		SqlQuery<Integer> query = new MappingSqlQueryWithParameters<Integer>() {
			...
			protected Integer mapRow(ResultSet rs, int rownum, @Nullable Object[] params, @Nullable Map<? ,?> context)
					throws SQLException {
				assertNull("params were null", params);
				assertNull("context was null", context);
				return rs.getInt(1);
			}
		};
		...
		assertThat(list, is(equalTo(Collections.singletonList(1))));
	...
	public void testFindCustomerIntInt() throws SQLException {
		...
		assertEquals("Customer id was assigned correctly", 1, cust.getId());
		assertEquals("Customer forename was assigned correctly", "rod", cust.getForename());
	...
	public void testFindCustomerMixed() throws SQLException {
		...
		assertNotNull("Found customer", cust1);
		assertEquals("Customer id was assigned correctly", 1, cust1.getId());

		Customer cust2 = query.findCustomer(1, "Roger");
		assertNull("No customer found", cust2);
	...
	public void testListCustomersIntInt() throws SQLException {
		...
		assertEquals("2 results in list", 2, list.size());
