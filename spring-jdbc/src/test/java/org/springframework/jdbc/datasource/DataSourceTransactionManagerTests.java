...
public class DataSourceTransactionManagerTests  {
	...
	private void doTestTransactionCommitRestoringAutoCommit(
		...
		assertFalse("Hasn't thread connection", TransactionSynchronizationManager.hasResource(dsToUse));
	...
	private void doTestTransactionRollbackRestoringAutoCommit(
		...
		try {
			...
			// expected
			assertEquals("Correct exception thrown", ex2, ex);
		}

		assertFalse("Hasn't thread connection", TransactionSynchronizationManager.hasResource(ds));
		assertFalse("Synchronization not active", TransactionSynchronizationManager.isSynchronizationActive());
		...
		try {
			tt.execute(new TransactionCallbackWithoutResult() {
				...
				protected void doInTransactionWithoutResult(TransactionStatus status) throws RuntimeException {
					...
					assertFalse("Synchronization not active",
							TransactionSynchronizationManager.isSynchronizationActive());
					assertFalse("Is existing transaction", status.isNewTransaction());
					throw ex;
				}
			});
			fail("Should have thrown RuntimeException");
	...
	public void testPropagationRequiresNewWithExistingTransactionAndUnrelatedDataSource() throws Exception {
		...
		assertFalse("Hasn't thread connection", TransactionSynchronizationManager.hasResource(ds));
		assertFalse("Hasn't thread connection", TransactionSynchronizationManager.hasResource(ds2));
		assertFalse("Synchronization not active", TransactionSynchronizationManager.isSynchronizationActive());
			...
			protected void doInTransactionWithoutResult(TransactionStatus status) throws RuntimeException {
				...
				tt.execute(new TransactionCallbackWithoutResult() {
					...
					protected void doInTransactionWithoutResult(TransactionStatus status) throws RuntimeException {
						assertFalse("Hasn't thread connection", TransactionSynchronizationManager.hasResource(ds));
						assertTrue("Synchronization active", TransactionSynchronizationManager.isSynchronizationActive());
						assertFalse("Isn't new transaction", status.isNewTransaction());
						assertFalse(TransactionSynchronizationManager.isCurrentTransactionReadOnly());
						assertFalse(TransactionSynchronizationManager.isActualTransactionActive());
						status.setRollbackOnly();
					}
	...
	public void testTransactionWithPropagationSupports() throws Exception {
		...
		tt.execute(new TransactionCallbackWithoutResult() {
			...
			protected void doInTransactionWithoutResult(TransactionStatus status) throws RuntimeException {
				assertFalse("Hasn't thread connection", TransactionSynchronizationManager.hasResource(ds));
				assertFalse("Is not new transaction", status.isNewTransaction());
				assertFalse(TransactionSynchronizationManager.isCurrentTransactionReadOnly());
				assertFalse(TransactionSynchronizationManager.isActualTransactionActive());
			}
		});
	...
	private void doTestExistingTransactionWithPropagationNested(final int count) throws Exception {
		...
		tt.execute(new TransactionCallbackWithoutResult() {
			...
			protected void doInTransactionWithoutResult(TransactionStatus status) throws RuntimeException {
				assertTrue("Is new transaction", status.isNewTransaction());
				assertFalse("Isn't nested transaction", status.hasSavepoint());
						...
						protected void doInTransactionWithoutResult(TransactionStatus status) throws RuntimeException {
							...
							ntt.execute(new TransactionCallbackWithoutResult() {
								...
								protected void doInTransactionWithoutResult(TransactionStatus status) throws RuntimeException {
									...
									assertTrue("Synchronization active", TransactionSynchronizationManager.isSynchronizationActive());
									assertFalse("Isn't new transaction", status.isNewTransaction());
									assertFalse("Is regular transaction", status.hasSavepoint());
