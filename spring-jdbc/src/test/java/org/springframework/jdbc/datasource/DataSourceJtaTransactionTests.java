...
public class DataSourceJtaTransactionTests {
	...
	private void doTestJtaTransaction(final boolean rollback) throws Exception {
		...
		assertFalse("Hasn't thread connection", TransactionSynchronizationManager.hasResource(dataSource));
		assertFalse("JTA synchronizations not active", TransactionSynchronizationManager.isSynchronizationActive());
			...
			protected void doInTransactionWithoutResult(TransactionStatus status) throws RuntimeException {
				...
				for (int i = 0; i < 5; i++) {
					...
					tt.execute(new TransactionCallbackWithoutResult() {
						@Override
						protected void doInTransactionWithoutResult(TransactionStatus status) throws RuntimeException {
							assertFalse("Hasn't thread connection",
