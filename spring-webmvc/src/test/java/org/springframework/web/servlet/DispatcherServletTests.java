...
public class DispatcherServletTests {
	...
	public void configuredDispatcherServlets() {
		assertEquals("Correct namespace", ("simple" + FrameworkServlet.DEFAULT_NAMESPACE_SUFFIX),
				simpleDispatcherServlet.getNamespace());
		assertEquals("Correct attribute", (FrameworkServlet.SERVLET_CONTEXT_PREFIX + "simple"),
				simpleDispatcherServlet.getServletContextAttributeName());
		assertSame("Context published", simpleDispatcherServlet.getWebApplicationContext(),
				getServletContext().getAttribute(FrameworkServlet.SERVLET_CONTEXT_PREFIX + "simple"));

		assertEquals("Correct namespace", "test", complexDispatcherServlet.getNamespace());
		assertEquals("Correct attribute", (FrameworkServlet.SERVLET_CONTEXT_PREFIX + "complex"),
				complexDispatcherServlet.getServletContextAttributeName());
		assertNull("Context not published",
	...
	public void invalidRequest() throws Exception {
		...
		assertEquals("correct error code", HttpServletResponse.SC_NOT_FOUND, response.getStatus());
	}
	...
	public void parameterizableViewController() throws Exception {
		...
		MockHttpServletResponse response = new MockHttpServletResponse();
		complexDispatcherServlet.service(request, response);
		assertEquals("forwarded to form", "myform.jsp", response.getForwardedUrl());
	}
	...
	public void unknownRequest() throws Exception {
		...
		assertEquals("Exception exposed", request.getAttribute("exception").getClass(), ServletException.class);
	}
	...
	public void anotherLocaleRequest() throws Exception {
		...
		assertNull("Not forwarded", response.getForwardedUrl());
		assertNotNull(request.getAttribute("test1"));
		assertNull(request.getAttribute("test1x"));
		assertNull(request.getAttribute("test1y"));
		assertNotNull(request.getAttribute("test2"));
		assertNull(request.getAttribute("test2x"));
		assertNull(request.getAttribute("test2y"));
		assertNotNull(request.getAttribute("test3"));
		assertNotNull(request.getAttribute("test3x"));
		assertNotNull(request.getAttribute("test3y"));
	...
	public void handlerInterceptorAbort() throws Exception {
		...
		assertNull("Not forwarded", response.getForwardedUrl());
		assertNotNull(request.getAttribute("test1"));
		assertNotNull(request.getAttribute("test1x"));
		assertNull(request.getAttribute("test1y"));
		assertNull(request.getAttribute("test2"));
		assertNull(request.getAttribute("test2x"));
		assertNull(request.getAttribute("test2y"));
	}
	...
	public void simpleMappingExceptionResolverWithDefaultErrorView() throws Exception {
		...
		assertEquals(200, response.getStatus());
		assertEquals("forwarded to failed", "failed0.jsp", response.getForwardedUrl());
		assertEquals("Exception exposed", request.getAttribute("exception").getClass(), RuntimeException.class);
	}
	...
	public void notAuthorized() throws Exception {
		...
		try {
			complexDispatcherServlet.service(request, response);
			assertEquals("Correct response", HttpServletResponse.SC_FORBIDDEN, response.getStatus());
		}
		catch (ServletException ex) {
			fail("Should not have thrown ServletException: " + ex.getMessage());
		}
	...
	public void detectHandlerMappingFromParent() throws ServletException, IOException {
		...
		assertNotEquals("Matched through parent controller/handler pair: not response=" + response.getStatus(),
	...
	public void noHandlerFoundExceptionMessage() {
		...
		assertFalse(ex.getMessage().contains("bar"));
		assertFalse(ex.toString().contains("bar"));
	}
