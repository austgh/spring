...
public class BindTagTests extends AbstractTagTests {
	...
	public void bindTagWithoutErrors() throws JspException {
		...
		tag.setPath("tb");
		assertEquals("Correct doStartTag return value", Tag.EVAL_BODY_INCLUDE, tag.doStartTag());
		BindStatus status = (BindStatus) pc.getAttribute(BindTag.STATUS_VARIABLE_NAME, PageContext.REQUEST_SCOPE);
		assertNotNull("Has status variable", status);
		assertNull("Correct expression", status.getExpression());
		assertNull("Correct value", status.getValue());
		assertEquals("Correct displayValue", "", status.getDisplayValue());
		assertFalse("Correct isError", status.isError());
		assertEquals("Correct errorCodes", 0, status.getErrorCodes().length);
		assertEquals("Correct errorMessages", 0, status.getErrorMessages().length);
		assertEquals("Correct errorCode", "", status.getErrorCode());
		assertEquals("Correct errorMessage", "", status.getErrorMessage());
		assertEquals("Correct errorMessagesAsString", "", status.getErrorMessagesAsString(","));
	}
	...
	public void bindTagWithGlobalErrors() throws JspException {
		...
		assertEquals("Correct errorCodes", 1, status.getErrorCodes().length);
		assertEquals("Correct errorMessages", 1, status.getErrorMessages().length);
		assertEquals("Correct errorCode", "code1", status.getErrorCode());
		assertEquals("Correct errorMessage", "message1", status.getErrorMessage());
		assertEquals("Correct errorMessagesAsString", "message1", status.getErrorMessagesAsString(","));
		...
		assertNotNull("Has status variable", status);
		assertEquals("Correct expression", "*", status.getExpression());
		assertNull("Correct value", status.getValue());
		assertEquals("Correct displayValue", "", status.getDisplayValue());
		assertTrue("Correct isError", status.isError());
		assertEquals("Correct errorCodes", 1, status.getErrorCodes().length);
	...
	public void bindTagWithFieldErrors() throws JspException {
		...
		BindStatus status = (BindStatus) pc.getAttribute(BindTag.STATUS_VARIABLE_NAME, PageContext.REQUEST_SCOPE);
		assertNotNull("Has status variable", status);
		assertEquals("Correct expression", "name", status.getExpression());
		assertEquals("Correct value", "name1", status.getValue());
		assertEquals("Correct displayValue", "name1", status.getDisplayValue());
		assertTrue("Correct isError", status.isError());
		assertEquals("Correct errorCodes", 2, status.getErrorCodes().length);
		assertEquals("Correct errorMessages", 2, status.getErrorMessages().length);
		assertEquals("Correct errorCode", "code1", status.getErrorCode());
		assertEquals("Correct errorCode", "code1", status.getErrorCodes()[0]);
		assertEquals("Correct errorCode", "code2", status.getErrorCodes()[1]);
		assertEquals("Correct errorMessage", "message &amp; 1", status.getErrorMessage());
		assertEquals("Correct errorMessage", "message &amp; 1", status.getErrorMessages()[0]);
		assertEquals("Correct errorMessage", "message2", status.getErrorMessages()[1]);
		assertEquals("Correct errorMessagesAsString", "message &amp; 1 - message2", status.getErrorMessagesAsString(" " +
				"- "));
		...
		status = (BindStatus) pc.getAttribute(BindTag.STATUS_VARIABLE_NAME, PageContext.REQUEST_SCOPE);
		assertNotNull("Has status variable", status);
		assertEquals("Correct expression", "age", status.getExpression());
		assertEquals("Correct value", new Integer(0), status.getValue());
		assertEquals("Correct displayValue", "0", status.getDisplayValue());
		assertTrue("Correct isError", status.isError());
		assertEquals("Correct errorCodes", 1, status.getErrorCodes().length);
		assertEquals("Correct errorMessages", 1, status.getErrorMessages().length);
		assertEquals("Correct errorCode", "code2", status.getErrorCode());
		assertEquals("Correct errorMessage", "message2", status.getErrorMessage());
		assertEquals("Correct errorMessagesAsString", "message2", status.getErrorMessagesAsString(" - "));
		...
		assertEquals("Correct displayValue", "", status.getDisplayValue());
		assertTrue("Correct isError", status.isError());
		assertEquals("Correct errorCodes", 3, status.getErrorCodes().length);
		assertEquals("Correct errorMessages", 3, status.getErrorMessages().length);
		assertEquals("Correct errorCode", "code1", status.getErrorCode());
		assertEquals("Correct errorCode", "code1", status.getErrorCodes()[0]);
		assertEquals("Correct errorCode", "code2", status.getErrorCodes()[1]);
		assertEquals("Correct errorCode", "code2", status.getErrorCodes()[2]);
		assertEquals("Correct errorMessage", "message & 1", status.getErrorMessage());
		assertEquals("Correct errorMessage", "message & 1", status.getErrorMessages()[0]);
		assertEquals("Correct errorMessage", "message2", status.getErrorMessages()[1]);
		assertEquals("Correct errorMessage", "message2", status.getErrorMessages()[2]);
	}
	...
	public void bindTagWithNestedFieldErrors() throws JspException {
		...
		BindStatus status = (BindStatus) pc.getAttribute(BindTag.STATUS_VARIABLE_NAME, PageContext.REQUEST_SCOPE);
		assertNotNull("Has status variable", status);
		assertEquals("Correct expression", "spouse.name", status.getExpression());
		assertEquals("Correct value", "name2", status.getValue());
		assertEquals("Correct displayValue", "name2", status.getDisplayValue());
		assertTrue("Correct isError", status.isError());
		assertEquals("Correct errorCodes", 1, status.getErrorCodes().length);
		assertEquals("Correct errorMessages", 1, status.getErrorMessages().length);
		assertEquals("Correct errorCode", "code1", status.getErrorCode());
	...
	public void bindTagWithIndexedProperties() throws JspException {
		...
		BindStatus status = (BindStatus) pc.getAttribute(BindTag.STATUS_VARIABLE_NAME, PageContext.REQUEST_SCOPE);
		assertNotNull("Has status variable", status);
		assertEquals("Correct expression", "array[0]", status.getExpression());
		assertTrue("Value is TestBean", status.getValue() instanceof TestBean);
		assertEquals("Correct value", "name0", ((TestBean) status.getValue()).getName());
		assertTrue("Correct isError", status.isError());
		assertEquals("Correct errorCodes", 2, status.getErrorCodes().length);
		assertEquals("Correct errorMessages", 2, status.getErrorMessages().length);
		assertEquals("Correct errorCode", "code1", status.getErrorCodes()[0]);
		assertEquals("Correct errorCode", "code2", status.getErrorCodes()[1]);
		assertEquals("Correct errorMessage", "message1", status.getErrorMessages()[0]);
		assertEquals("Correct errorMessage", "message2", status.getErrorMessages()[1]);
	}
	...
	public void bindTagWithMappedProperties() throws JspException {
		...
		BindStatus status = (BindStatus) pc.getAttribute(BindTag.STATUS_VARIABLE_NAME, PageContext.REQUEST_SCOPE);
		assertNotNull("Has status variable", status);
		assertEquals("Correct expression", "map[key1]", status.getExpression());
		assertTrue("Value is TestBean", status.getValue() instanceof TestBean);
		assertEquals("Correct value", "name4", ((TestBean) status.getValue()).getName());
		assertTrue("Correct isError", status.isError());
		assertEquals("Correct errorCodes", 2, status.getErrorCodes().length);
		assertEquals("Correct errorMessages", 2, status.getErrorMessages().length);
		assertEquals("Correct errorCode", "code1", status.getErrorCodes()[0]);
	...
	public void bindTagWithIndexedPropertiesAndCustomEditor() throws JspException {
		...
		assertEquals("Correct value", "something", status.getValue());
	}
	...
	public void bindErrorsTagWithoutErrors() throws JspException {
		...
		tag.setPageContext(pc);
		tag.setName("tb");
		assertEquals("Correct doStartTag return value", Tag.SKIP_BODY, tag.doStartTag());
		assertNull("Doesn't have errors variable", pc.getAttribute(BindErrorsTag.ERRORS_VARIABLE_NAME));
	}
	...
	public void bindErrorsTagWithErrors() throws JspException {
		...
		tag.setName("tb");
		assertEquals("Correct doStartTag return value", Tag.EVAL_BODY_INCLUDE, tag.doStartTag());
		assertSame("Has errors variable", pc.getAttribute(BindErrorsTag.ERRORS_VARIABLE_NAME,
				PageContext.REQUEST_SCOPE), errors);
	}
	...
	public void nestedPathWithBindTag() throws JspException {
		...
		assertEquals("Correct doStartTag return value", Tag.EVAL_BODY_INCLUDE, bindTag.doStartTag());
		BindStatus status = (BindStatus) pc.getAttribute(BindTag.STATUS_VARIABLE_NAME, PageContext.REQUEST_SCOPE);
		assertNotNull("Has status variable", status);
		...
		assertEquals("Correct doStartTag return value", Tag.EVAL_BODY_INCLUDE, bindTag2.doStartTag());
		BindStatus status2 = (BindStatus) pc.getAttribute(BindTag.STATUS_VARIABLE_NAME, PageContext.REQUEST_SCOPE);
		assertNotNull("Has status variable", status2);
