...
public abstract class AbstractBeanFactoryTests {
	...
	public void inheritance() {
		...
		assertEquals("rod.name is Rod", "Rod", rod.getName());
	...
	public void getInstanceByNonmatchingClass() {
		try {
			...
			// So far, so good
			assertEquals("Exception has correct bean name", "rod", ex.getBeanName());
			assertEquals("Exception requiredType must be BeanFactory.class", ex.getRequiredType(), BeanFactory.class);
			assertTrue("Exception actualType as TestBean.class", TestBean.class.isAssignableFrom(ex.getActualType()));
			assertSame("Actual type is correct", ex.getActualType(), getBeanFactory().getBean("rod").getClass());
		}
	}
	...
	public void sharedInstancesAreEqual() {
		...
		assertSame("Object equals applies", o, o1);
	}
	...
	public void prototypeInstancesAreIndependent() {
		...
		assertNotSame("ref equal DOES NOT apply", tb1, tb2);
		assertEquals("object equal true", tb1, tb2);
		tb1.setAge(1);
		tb2.setAge(2);
		assertEquals("1 age independent = 1", 1, tb1.getAge());
		assertEquals("2 age independent = 2", 2, tb2.getAge());
		assertFalse("object equal now false", tb1.equals(tb2));
	}
	...
	public void xtestTypeMismatch() {
		try {
			...
			// Further tests
			assertEquals("Has one error ", 1, ex.getExceptionCount());
			assertNotNull("Error is for field age", ex.getPropertyAccessException("age"));
			assertEquals("We have rejected age in exception", "34x",
					ex.getPropertyAccessException("age").getPropertyChangeEvent().getNewValue());
		}
	}
	...
	public void factorySingleton() throws Exception {
		...
		assertEquals("Singleton from factory has correct name, not " + tb.getName(), DummyFactory.SINGLETON_NAME,
				tb.getName());
		DummyFactory factory = (DummyFactory) getBeanFactory().getBean("&singletonFactory");
		TestBean tb2 = (TestBean) getBeanFactory().getBean("singletonFactory");
		assertSame("Singleton references ==", tb, tb2);
		assertNotNull("FactoryBean is BeanFactoryAware", factory.getBeanFactory());
	}
	...
	public void factoryPrototype() throws Exception {
		...
		assertFalse(tb.getName().equals(DummyFactory.SINGLETON_NAME));
	...
	public void aliasing() {
		...
		try {
			...
			// Ok
			assertEquals(alias, ex.getBeanName());
		}
