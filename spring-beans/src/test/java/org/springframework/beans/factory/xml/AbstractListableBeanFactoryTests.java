...
public abstract class AbstractListableBeanFactoryTests extends AbstractBeanFactoryTests {
	...
	protected final void assertCount(int count) {
		...
		assertEquals("We should have " + count + " beans, not " + defnames.length, defnames.length, count);
	}
	...
	protected void assertTestBeanCount(int count) {
		...
		assertEquals("We should have " + count + " beans for class org.springframework.tests.sample.beans.TestBean, " +
				"not " +
		...
		assertEquals("We should have " + countIncludingFactoryBeans +
				" beans for class org.springframework.tests.sample.beans.TestBean, not " + names.length, names.length,
	...
	public void getDefinitionsForNoSuchClass() {
		...
		assertEquals("No string definitions", 0, defnames.length);
	}
	...
	public void getCountForFactoryClass() {
		assertEquals("Should have 2 factories, not " +
