...
public class BeanWrapperTests extends AbstractPropertyAccessorTests {
	...
	public void setterDoesNotCallGetter() {
		...
		assertEquals("Set name to tom", "tom", target.getName());
	}
	...
	public void aliasedSetterThroughDefaultMethod() {
		...
		assertEquals("Set name to tom", "tom", target.getAliasedName());
	}
	...
	public void setValidAndInvalidPropertyValuesShouldContainExceptionDetails() {
		...
		try {
			...
			assertEquals("Must contain 2 exceptions", 2, ex.getExceptionCount());
			// Test validly set property matches
			assertEquals("Vaid set property must stick", target.getName(), newName);
			assertEquals("Invalid set property must retain old value", 0, target.getAge());
			assertEquals("New value of dodgy setter must be available through exception",
					ex.getPropertyAccessException("touchy").getPropertyChangeEvent().getNewValue(), invalidTouchy);
		}
	}
	...
	public void setPropertyTypeMismatch() {
		...
		assertEquals(8, (int) target.getObject());
