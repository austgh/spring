...
public class MutablePropertyValuesTests extends AbstractPropertyValuesTests {
	...
	public void testAddOrOverride() {
        ...
        assertEquals(pvs.getPropertyValue("rod"), addedPv);
		PropertyValue changedPv = new PropertyValue("forname", "Greg");
		pvs.addPropertyValue(changedPv);
        assertEquals(pvs.getPropertyValue("forname"), changedPv);
	}
	...
	public void testChangesOnEquals() {
        ...
        assertEquals("changes are empty", 0, changes.getPropertyValues().length);
	}
	...
	public void testChangeOfOneField() {
		...
		PropertyValues changes = pvs2.changesSince(pvs);
        assertEquals("changes are empty, not of length " + changes.getPropertyValues().length, 0,
                changes.getPropertyValues().length);
        ...
        assertNotNull("change is forname", fn);
        assertEquals("new value is gordon", "Gordon", fn.getValue());
		...
		changes = pvs3.changesSince(pvs);
        assertEquals("2 change", 2, changes.getPropertyValues().length);
		fn = changes.getPropertyValue("foo");
        assertNotNull("change in foo", fn);
        assertEquals("new value is bar", "bar", fn.getValue());
	}
